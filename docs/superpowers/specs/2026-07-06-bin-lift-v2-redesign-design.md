...
## Milestone 2 review — carried-forward decisions

Milestone 2 (`lift-llvmkit`, the pure-Rust IR backend over llvmkit v0.0.3) is implemented and merged (8 commits; llvmkit builds on stable rustc 1.96.1; 30 golden `.ll` tests + `verify()` confirms well-formed IR; clippy clean). Its whole-branch review surfaced two forward decisions that **gate Milestone 3**:

- **GATING M3 — resolve `Session` ↔ `LlvmkitBuilder` ownership before writing x86 handlers.** `lift-core`'s `Session` owns `builder: L::Builder` *by value* and drives `step()`/`resume()` across many calls, but `LlvmkitBuilder<'m,'ctx,B>` borrows a `&'m Module` that only exists inside the `Module::with_new(|m| …)` generative-brand closure — llvmkit has **no owned-`Module` constructor**, so a `Session` holding an `LlvmkitBuilder` cannot outlive the closure, and M1's `Session::new(lifter, builder, …)` signature can't be satisfied. **M3 must open with a decision spike** choosing among: (a) invert control — run the whole `Session` lifecycle *inside* `with_new` (conflicts with streaming/suspend, since the closure can't yield across `Oracle` round-trips); (b) `Session` stops owning the builder — pass `&mut impl IrBuilder` into `step()` per call (likely cleanest for streaming); or (c) grow an owned-`Module` variant in the backend. This shapes all of M3.
- **`position_at` is single-shot per block (documented invariant).** A block is positioned exactly once and then terminated; it cannot be re-entered/appended-to. Loop back-edges / block revisits panic by construction. M3 must emit each block's full body in one positioned span; if deferred block completion (e.g. PHI fixups) is ever needed, the backend needs a non-consuming reposition variant.
- **Width discipline will bite at `.expect()` in M3.** The data-flow `IrBuilder` methods return values (not `Result`) and the backend `.expect()`s llvmkit's fallible ops, which runtime-check width equality. Every sub-register (8/16/32-bit) x86 op must explicitly `zext/sext/trunc` operands to a common width *before* `iadd`/`icmp`/`select`, or the `.expect()` fires as a hard panic. Consider a width-bookkeeping helper (or a diagnostic debug-assert) in `lift-x86`.
- **Other forward notes:** `switch` scrutinee/case-consts are 64-bit-only (fine for address switches; generalize if M3/M5 needs narrow-value switches); `build_ret` type-checks against the declared return width (test/handler functions must return exactly that width); deferred to M4 — the `store` value type-check and the redundant per-access `inttoptr` (subsumed by the `MemoryModel`); revisit `const_int`'s `value as i64` cast for full-range 64-bit immediates in M3.
