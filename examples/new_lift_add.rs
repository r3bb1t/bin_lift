...
/// Lifts the sample add function to verified textual LLVM IR.
///
/// This variant stops decoding before `ret`, which is useful when inspecting
/// the straight-line body emitted by the lifter.
///
/// Source-level shape:
/// ```cpp
...
/// ```
///
/// The call is modeled as a call-side stack update. The printed output is the
/// protected function IR, not a complete executable `main`.
